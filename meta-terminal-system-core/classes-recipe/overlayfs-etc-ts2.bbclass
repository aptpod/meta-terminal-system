#
# Terminal System 2 custom overlayfs-etc-ts2 class
#
# This is a modified version of overlayfs-etc.bbclass from Yocto scarthgap.
# The only change is removing the IMAGE_FEATURES_CONFLICTS with package-management.
#
# Terminal System 2 uses Mender for system updates, and the rootfs is read-only.
# The package manager (dpkg/apt) is kept for debugging purposes (e.g., dpkg -l,
# dpkg -x) but not for installing packages to the rootfs, so the conflict is safe to bypass.
#
...
# Prevent the base overlayfs-etc class from being inherited via IMGCLASSES
IMGCLASSES:remove = "overlayfs-etc"

python __anonymous() {
    import hashlib

    # Verify that the base overlayfs-etc.bbclass hasn't changed
    base_class_path = d.expand("${COREBASE}/meta/classes-recipe/overlayfs-etc.bbclass")
    # Expected checksum for Yocto scarthgap (commit 6bdaf2aaefb2b2eaea21a49a4d5d6cc23c902dab)
    expected_checksum = "c68d296fb4e242a94f333ee589c93ad0dbc09f6d7e1fde128987a53bc447b10c"

    try:
        with open(base_class_path, 'rb') as f:
            actual_checksum = hashlib.sha256(f.read()).hexdigest()

        if actual_checksum != expected_checksum:
            bb.fatal(
                "Terminal System 2 custom overlayfs-etc.bbclass: "
                "The base overlayfs-etc.bbclass has changed!\n"
                "Expected checksum: %s\n"
                "Actual checksum:   %s\n"
                "Please review the changes in %s\n"
                "and update meta-terminal-system/meta-terminal-system-core/classes-recipe/overlayfs-etc.bbclass accordingly.\n"
                "Then update the expected_checksum in this file to: %s"
                % (expected_checksum, actual_checksum, base_class_path, actual_checksum)
            )
    except FileNotFoundError:
        bb.fatal("Base overlayfs-etc.bbclass not found at: %s" % base_class_path)
}

ROOTFS_POSTPROCESS_COMMAND += '${@bb.utils.contains("IMAGE_FEATURES", "overlayfs-etc", "create_overlayfs_etc_preinit", "", d)}'
# MODIFIED: Removed IMAGE_FEATURES_CONFLICTS to allow package-management with overlayfs-etc
# Original: IMAGE_FEATURES_CONFLICTS_overlayfs-etc = "${@ 'package-management' if bb.utils.to_boolean(d.getVar('OVERLAYFS_ETC_USE_ORIG_INIT_NAME'), True) else ''}"
