...
# And also include the following PR so that the dtb file is updated correctly when the kernel is updated.
# https://github.com/mendersoftware/meta-mender/pull/2114
...
# Mender does not use the kernel image in the boot partition, so it should be excluded.
# If the kernel image is included, the boot partition will not have enough space when updating the firmware, and the update may fail.
